...
// MVC 방식은 html로 응답 vs  REST 방싱은 데이터+ 상태코드로 응답
...
public class MemberController {
    ...
    @PostMapping(value = "/api/member/new",consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public  ResponseEntity<Member> singup(@ModelAttribute @Valid MemberDto.Create dto, BindingResult br){
        System.out.println(dto);
    ...
    @PutMapping("/api/members/fetchedPassword")
    ...
    //비밀번경 변경
    @PreAuthorize("isAuthenticated()")
    @Operation(summary = "비밀번호 변경", description = "기존 비밀번호, 새비밀번호로 비밀번호 변경")
    @PutMapping("/api/members/password")
    public ResponseEntity<String>chagePasswoed(@ModelAttribute @Valid MemberDto.PasswordChange dto , BindingResult bindingResult, Principal principal){
